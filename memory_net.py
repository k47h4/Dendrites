...
def run_network(params):
	...
	#we = .6*nS #.5*mV/(taum/tau_e)**(taum/(tau_e-taum))
	#wi = 3*nS #(el-el-lambdae*NE*we*tau_e)/(lambdae*NI*tau_i)
	#w_initial = 1e-10
	...
	last_bAP: second
	'''
	#normalisation = 0 = 1/100*(avg_weight_change + avg_weight_change_dend)/2.0 : 1 (summed)



	#dv/dt=(-gl*(v-el)-(ge*v*excitability+gi*(v-er)))*100*Mohm/taum : volt (unless refractory)
	#I_E = -ge*v : ampere
	#I_I = -gi*(v-er) : ampere
	#dge/dt = -ge/tau_e : siemens
	#dgi/dt = -gi/tau_i : siemens
	...
	#exc_neurons.run_regularly('instweightchange : 1', dt=.1*ms, when='synapses',order=0)
	...
	#connectionEE.on_pre_transmission.order = 1
	#on_pre={'on_pre_transmission': 'ge+=w*nS',
	#                            'on_pre_plasticity': on_pre_triplet},
	...
	#within_weights = StateMonitor(connectionEE,'w',record=True,dt=1000*ms)
	#outside_weights = StateMonitor(connectionEE,'w',record=[100:1000,100:1000], dt=100*ms)
	...
	#weight_pot.weight_pot = StateMonitor(connectionEE, 'weight_pot', record=[0])
	#weight_dep = StateMonitor(connectionEE, 'weight_dep', record=[0])
	...
	#for eta in np.arange(5):          # Learning rate
	...
	excitability_d = 1.0
	net = Network(collect())
	#warmuptime = 1.2*second#3*p.tau*second+1*second
	warmuptime=5*second
	...
	net.run(3*second)
	BrianLogger.log_level_info()
	#net.store('warmup_plastic')

	#net.restore('warmup_plastic') 
	#connection_input.active = True
	#net.run(simtime)
	if plot == True:
		plt.figure(figsize=(2.5,2.2))
	...
	#connectionEI.w = connectionEI.w*p.wINH#10 # 8 * 1.25
	...
	#print(np.shape(outside_inh_weights1.w))
	...
	if plot == True:
		...
		#a1.xaxis.set_major_locator(ticker.MultipleLocator(xtick_spacing))
		#a1.yaxis.set_major_locator(ticker.MultipleLocator(ytick_spacing))
		...
		print(np.sum(postsynapticweights.w,0)[1000:1100])
		print(np.shape(postsynapticweights.w)[1])
		print(np.shape(np.sum(postsynapticweights.w,0)))
		#plt.figure()
		#plt.plot(np.arange(np.shape(postsynapticweights.w)[1]),np.sum(postsynapticweights.w,0)[:])
		#plt.show()
		...
		#plt.figure()
		#plt.plot(np.arange(np.shape(avg_weight_change.avg_weight_change)[0]),avg_weight_change.avg_weight_change+avg_weight_change_dend.avg_weight_change_dend,color='k')
		#plt.show()
